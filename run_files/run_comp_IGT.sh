...
# CT3 for target copy, PT4 for target position, #O5 for origin
...
# CS5 for source copy, PS6 for source position, ML7 for morpheme length,
# S8 for seen words or DL8 for delexicalised morpheme
...
ARGS+=" --pattern 21:Gx/MM:0t0,0s0,1s0"
...
#ARGS+=" --pattern 49:Bx/LL:0t1,0s2,1s2" # Bigram length and binary category # NEW
#ARGS+=" --pattern 49:BB/LL:0t1,1t1,0s2,1s2" # Bigram length and binary category # NEW
...
# Comp features
#ARGS+=" --pattern 70:Ox/xx:0t5" # Label origin
#ARGS+=" --pattern 71:Ox/Mx:0t5,0s0" # Label origin and source morpheme
#ARGS+=" --pattern 72:BOx/Sx:0t1,0t5,0s8" # Binary, label origin and seen morpheme
#ARGS+=" --pattern 72:POx/MSx:0t2,0t5,0s0,0s8" # Binary, label origin and seen morpheme
ARGS+=" --pattern 73:Gx/DLx:0t0,0s8" # Delexicalised source morpheme
ARGS+=" --pattern 74:Bx/DLx:0t1,0s8" # Binary and delexicalised morpheme
ARGS+=" --pattern 75:Px/DLx:0t2,0s8" # PoS and delexicalised morpheme
ARGS+=" --pattern 76:Bx/DLDL:0t1,0s8,1s8" # Bigram binary and delexicalised morpheme # NEW
ARGS+=" --pattern 77:BB/DLDL:0t1,1t1,0s8,1s8" # Bigram binary and delexicalised morpheme # NEW

ARGS+=" --pattern 78:Bx/FMLx:0t1,0s1,0s7" # Morpheme position in the word, binary
...
lang='gitksan' #'natugu' #'gitksan' #'lezgi' #'tsez'
suffix='conc_match_comp_31' #'conc_match_comp_701_wodict' # 591 #'0_gold_match_1600' # Changer ici
...
#ARGS+=" --devel-out ../IGT_ST/dev/devel-${lang}-%02d-imt-${suffix}.txt"
...
#ARGS+=" --test-out  ../IGT_ST/output/output_${lang}_IGT_imt_${suffix}.out"
...
#ARGS+=" --mdl-load tsez_exp/model_imt_lost.wgh" # Multilingual pre-training
#ARGS+=" --str-load tsez_exp/model_imt_lost.str"
...
#ARGS+=" --ftr-dump tsez_exp/model_${lang}_IGT_${suffix}.ftr"
#ARGS+=" --ftr-dump tsez_exp/model_${lang}_IGT_IMT_${suffix}.ftr"
